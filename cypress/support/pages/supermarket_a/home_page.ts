export class SupermarketAHomePage {
    ...
    visitEggsPage():void  {
        cy.visit(`${Cypress.env('url_supermarket_A')}/oeufs-produits-laitiers/cremerie-oeufs-laits/oeufs/ca-n010103?redirect_keywords=oeufs&x-ua-device=mobile`)
    }
    ...
    checkPriceAscending():void {
        cy.get('article .product-price').then(($prices) => {
            let prices = $prices.map((i, el) => Cypress.$(el).text().replace('€', '').replace(',', '.')).get()
